...
export function validateProfile(
  ...
  if (
    ...
    && (capacityPct < VALIDATION_LIMITS.PROFILE_CAPACITY_MIN || capacityPct > VALIDATION_LIMITS.PROFILE_CAPACITY_MAX)
  ) {
    return {
      valid: false,
      error: `Capacity must be ${VALIDATION_LIMITS.PROFILE_CAPACITY_MIN}-${VALIDATION_LIMITS.PROFILE_CAPACITY_MAX}`,
    };
  }
  return { valid: true };
}
...
export function validateAssignment(
  ...
  if (
    dedicationPct < VALIDATION_LIMITS.ASSIGNMENT_DEDICATION_MIN
    || dedicationPct > VALIDATION_LIMITS.ASSIGNMENT_DEDICATION_MAX
  ) {
    return {
      valid: false,
      error: `Dedication must be ${VALIDATION_LIMITS.ASSIGNMENT_DEDICATION_MIN}-${VALIDATION_LIMITS.ASSIGNMENT_DEDICATION_MAX}`,
    };
  }
  return { valid: true };
}

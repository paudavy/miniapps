...
export function AssignmentBar({ assignment, profile, isOverloaded, laneIndex, laneCount }: Props): h.JSX.Element {
  ...
  const dragRef = useBarDrag({
    assignment,
    slotCount: count,
    onClick: openPopover,
  });

  const leftRef = useBarResize({
    assignment,
    slotCount: count,
    edge: 'left',
  });

  const rightRef = useBarResize({
    assignment,
    slotCount: count,
    edge: 'right',
  });
  ...
  return (
    <div
      ...
      className={`assignment-bar ${isOverloaded ? 'assignment-bar--overloaded overloaded' : ''}`}
      role="button"
      tabIndex={0}
      ...
      onMouseLeave={() => {
        if (hoveredBarState.value?.assignmentId === assignment.id) hoveredBarState.value = null;
      }}
      style={{
        left: `${left}px`,
        width: `${width - 2}px`,
    ...
    >
      <div ref={leftRef} className="assignment-bar__handle assignment-bar__handle--left bar-handle-left" />
      <div className="assignment-bar__body">
        <span className="assignment-bar__text assignment-bar__name">{assignment.task}</span>
      ...
      <div ref={rightRef} className="assignment-bar__handle assignment-bar__handle--right bar-handle-right" />

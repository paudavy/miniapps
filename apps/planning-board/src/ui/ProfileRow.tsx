...
export function ProfileRow({ profile, slotCount }: Props): h.JSX.Element {
  ...
  return (
    <div
      className={`profile-row ${isActive ? 'profile-row--active selected' : ''}`}
      role="button"
      tabIndex={0}
      ...
      style={{ '--row-height-current': `${rowHeight}px`, '--current-color': profile.color }}
    >
      <div className="profile-row__header">
        <div className="profile-row__avatar" style={{ background: profile.color }}>
          {profile.initials}
        </div>
        ...
        {overloads.length > 0 && (
          <span className="profile-row__badge">⚠ {overloads.length} slots</span>
        )}
        {isActive && (
          <>
            <input
      ...
      <div className="profile-row__slots">
        {Array.from({ length: slotCount }, (_, i) => (
          <SlotCell
            key={i}
            overloaded={overloadedSlots.has(i)}

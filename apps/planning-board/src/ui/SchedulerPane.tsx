...
export function SchedulerPane(): h.JSX.Element {
  ...
  const bodyHeight = profiles.value.reduce((total, profile) => {
    const profileAssignments = assignments.value.filter((assignment) => assignment.profileId === profile.id);
  ...
  return (
    <div className="scheduler-pane" onScroll={handleScroll}>
      <div className="scheduler-pane__header">
        ...
        {Array.from({ length: count }, (_, i) => (
          <SlotHeader
            ...
            slotIndex={i}
            viewMode={mode}
            isCurrent={visibleCurrentSlot === i}
            isMilestone={isMilestoneSlot(i)}
          />
        ))}
      </div>
      <div className="scheduler-pane__body">
        {visibleCurrentSlot !== null && profiles.value.length > 0 && (
          <div
            aria-hidden="true"
            className="scheduler-pane__current-line"
            style={{
              left: `calc(var(--row-header-width) + ${visibleCurrentSlot * width + width / 2}px)`,
              height: `${bodyHeight}px`,
            }}
          />
        )}
        {profiles.value.length === 0 ? (
          <div className="scheduler-pane__empty">

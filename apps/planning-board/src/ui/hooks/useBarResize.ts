    ...
    const dragBehavior = drag<HTMLDivElement, unknown>()
      ...
      .on('end', (event) => {
        ...
        if (edge === 'left') {
          ...
          const clamped = Math.min(newStart, assignment.endSlot);
          updateAssignment(assignment.id, { startSlot: clamped });
        } else {
          const newEnd = clampSlot(origSlot + slotDelta, 0, slotCount - 1);
          const clamped = Math.max(newEnd, assignment.startSlot);
          updateAssignment(assignment.id, { endSlot: clamped });
        }
      });

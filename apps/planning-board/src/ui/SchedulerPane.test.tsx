...
import { profiles, viewMode, slotCount, currentSlot, editingAssignmentId, contextMenuTarget, hoveredBarState } from '../state/signals';
...
describe('SchedulerPane', () => {
  ...
  it('renders current slot line when currentSlot is within rendered range and rows exist', () => {
    profiles.value = [{ id: 'p1', name: 'Alice', category: '', capacityPct: 100, color: '#4f9cf8', initials: 'AL' }];
    currentSlot.value = 4;

    const { container, getAllByTestId } = render(<SchedulerPane />);
    expect(container.querySelector('.scheduler-pane__current-line')).toBeTruthy();
